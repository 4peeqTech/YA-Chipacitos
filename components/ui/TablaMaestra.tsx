    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-muted text-sm mt-0.5">{descripcion}</p>
        </div>
        <button
          onClick={abrirCrear}
          className="shrink-0 flex items-center gap-1.5 bg-accent hover:opacity-90 text-black font-bold text-sm py-2 px-4 rounded-xl transition-all"
        >
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-bg border border-border text-text rounded-xl pl-9 pr-3 py-2 text-sm focus:outline-none focus:border-accent transition-colors"
        />
      </div>
          ...
          <div className="divide-y divide-surface2">
            {[0, 1, 2].map(i => (
              <div key={i} className="px-4 py-3.5 flex items-center gap-3 animate-pulse">
                ...
                <div className="h-5 bg-surface2 rounded-full w-16 ml-auto" />
              </div>
            ))}
          </div>
        ) : filtrados.length === 0 ? (
              ...
              <div key={item.id} className={`px-4 py-3 flex items-center gap-3 ${!item.activo ? 'opacity-50' : ''}`}>
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border transition-colors shrink-0 ${
                    item.activo ? 'bg-green-bg border-success text-success' : 'bg-surface2 border-border text-faint'
                  }`}
                >
                  <span className={`w-1.5 h-1.5 rounded-full ${item.activo ? 'bg-success' : 'bg-faint'}`} />
                ...
                <button
                  ...
                  className="w-8 h-8 shrink-0 flex items-center justify-center rounded-lg text-muted hover:text-accent hover:bg-surface2 transition-colors"
                >
                  <Pencil size={15} />
                </button>
                <button
                  ...
                  className="w-8 h-8 shrink-0 flex items-center justify-center rounded-lg text-muted hover:text-red-400 hover:bg-red-900/20 transition-colors"
                >
                  <Trash2 size={15} />
                </button>
              </div>
      ...
      <Modal open={modal !== null} onClose={cerrarModal} title={modal === 'editar' ? `Editar ${nombreSingular}` : `Nuevo ${nombreSingular}`}>
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-accent uppercase tracking-wider mb-1.5">Nombre</label>
            <input
              autoFocus
          ...
          {(camposExtra ?? [])
            ...
            .map(c => (
              <div key={c.key}>
                {c.tipo === 'numero' && (
                  <>
                    <label className="block text-xs font-semibold text-accent uppercase tracking-wider mb-1.5">{c.label}</label>
                    <InputNumero
                      placeholder="0"
                      className={controlClass}
                ...
                {c.tipo === 'checkbox' && (
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <label htmlFor={`campo-extra-${c.key}`} className="text-sm text-text flex items-center cursor-pointer">
                      {c.label}
          ...
          <div className="flex gap-2 pt-1">
            <button onClick={cerrarModal} className="flex-1 py-2.5 border border-border rounded-xl text-sm font-medium text-muted hover:text-text transition-colors">
              Cancelar
            </button>
            <button
              onClick={guardar}
              disabled={guardando || !formNombre.trim() || numerosFaltantes.length > 0}
              className="flex-1 py-2.5 bg-accent text-black rounded-xl text-sm font-bold disabled:opacity-40 transition-opacity"
            >
      ...
      <Modal open={!!eliminando} onClose={() => setEliminando(null)} title="Eliminar" accent="red">
        <p className="text-sm text-muted">
          ¿Eliminar <span className="text-text font-medium">{eliminando?.nombre}</span>? Esta acción no se puede deshacer.
        </p>
        <div className="flex gap-2 pt-4">
          <button onClick={() => setEliminando(null)} className="flex-1 py-2.5 border border-border rounded-xl text-sm font-medium text-muted hover:text-text transition-colors">
            Cancelar
          </button>
