...
-- Reconstruido desde el estado real de producción: este bootstrap se
-- aplicó antes de que existiera tracking de migraciones (supabase_migrations
-- no tiene entrada para él), por eso se fecha antes que el resto del historial.
-- ================================================
...
-- (superada más adelante por conciliacion_usar_remito / conciliacion_agregar_montos)
-- ================================================
...
-- Productos (todos los autenticados pueden ver)
create policy "autenticados ven productos" on productos
  for select using (auth.uid() is not null);

-- Nota: en algún momento posterior no registrado en el historial de
-- migraciones, las policies de gestión se acotaron para que cada rol
-- solo administre productos de su propio destino. Se refleja el estado
-- real de producción.
create policy "fabrica gestiona sus productos" on productos
  for all using (
    get_user_rol() = 'admin' or (get_user_rol() = 'fabrica' and destino = 'fabrica')
  );

create policy "deposito gestiona sus insumos" on productos
  for all using (
    get_user_rol() = 'admin' or (get_user_rol() = 'deposito' and destino = 'deposito')
  );
