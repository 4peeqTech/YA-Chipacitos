-- ================================================
-- Permitir eliminar usuarios desde el módulo de admin
-- ================================================
-- El admin no podía borrar usuarios: profiles.id no tenía
-- ON DELETE CASCADE hacia auth.users, y varias tablas históricas
-- (pedidos, ventas, gastos, tareas, informes) referenciaban
-- profiles(id) sin acción de borrado. Postgres rechazaba el
-- DELETE por violación de foreign key.
--
-- Fix: al borrar el auth.user se borra en cascada su profile, y
-- los registros históricos que lo referencian quedan con esa
-- columna en null (no se pierde el historial, solo se desvincula
...
-- gastos.created_by (ya era set null; se recrea para dejarlo explícito e idéntico al resto)
